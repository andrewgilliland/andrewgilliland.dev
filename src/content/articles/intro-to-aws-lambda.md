...
Lambda is not a general-purpose compute platform. Several constraints make it the wrong choice for specific workloads.

**The 15-minute timeout is a hard ceiling.** Lambda cannot run longer than 15 minutes. If your process involves long-running data transformations, batch jobs, model training, or multi-step workflows that exceed this window, Lambda can't do it alone. Use Step Functions to orchestrate Lambda calls across multiple steps, or use ECS Fargate for tasks that need to run as long as they need to run.
...
**Lambda is not a persistent connection server.** You can't host a WebSocket server on Lambda. You can't maintain a long-lived streaming connection. Lambda's execution model is fundamentally request/response or event/response — one invocation, one lifecycle. For persistent connections, use API Gateway WebSocket APIs (which manage connection state externally) or a server-based compute tier.
...
**Observability requires deliberate setup.** Lambda doesn't give you distributed tracing, structured log correlation, or performance profiling by default. CloudWatch Logs captures stdout/stderr, but correlating logs across function invocations, debugging cold starts, and tracing a request across multiple Lambda functions requires X-Ray, Lambda Powertools, and intentional instrumentation.
...
The CDK construct for Lambda is `aws_lambda.Function`. At minimum, you need a runtime, a handler path, and a code asset pointing to a directory containing your function code.

### Python Handler (Python 3.13 Runtime)
...
### TypeScript Handler (Node.js 22 Runtime)

Same CDK construct, different runtime. Swap `PYTHON_3_13` for `NODEJS_22_X`:
